                ...
                <tbody class="divide-y divide-slate-300">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-right">
                            @if($step == 3)
                                <a href="{{ route('reports.pdf', $report->id) }}" class="inline-flex items-center justify-center px-5 py-2.5 text-[10px] font-black text-slate-700 bg-white border-2 border-slate-300 hover:bg-slate-50 hover:ring-4 hover:ring-slate-200 rounded-lg transition-all shadow-sm cursor-pointer">
...
<div id="reportModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-[100] justify-center items-center w-full md:inset-0 h-screen max-h-full bg-slate-900/60 backdrop-blur-md transition-opacity">
    ...
    <div class="relative p-4 w-full max-w-5xl max-h-full mx-auto">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl flex flex-col w-full max-h-[90vh] border-2 border-slate-300 pointer-events-auto overflow-hidden">
            ...
            <div class="flex items-center justify-end gap-3 p-4 md:p-5 border-t-2 border-slate-200 bg-white shrink-0 z-20 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
...
<script>
    // Memindahkan modal ke luar dari kurungan animasi agar z-index & blur sempurna
    ...
    function openReportModal(vesselId, vesselName, reportData) {
        ...
        if(reportData) {
            ...
            textareas.forEach(field => {
                if(reportData[field]) {
                    document.querySelector(`[name="${field}"]`).value = reportData[field];
                }
            });
        }
    }
...
<style>
    ...
    /* MEMATIKAN BACKDROP BAWAAN FLOWBITE AGAR BLUR KITA MUNCUL SEMPURNA */
    div[modal-backdrop] {
